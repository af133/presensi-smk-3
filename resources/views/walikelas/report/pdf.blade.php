<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }

    body {
        font-family: 'DejaVu Sans', sans-serif;
        font-size: 11px;
        color: #1a1a1a;
        background: #fff;
    }

    .page { padding: 36px 40px; }

    .header {
        border-bottom: 2.5px solid #1d4ed8;
        padding-bottom: 14px;
        margin-bottom: 20px;
    }
    .header-top {
        display: flex;
        justify-content: space-between;
        align-items: flex-start;
    }
    .school-name {
        font-size: 16px;
        font-weight: bold;
        color: #1d4ed8;
        letter-spacing: 0.3px;
    }
    .school-sub {
        font-size: 10px;
        color: #6b7280;
        margin-top: 2px;
    }
    .doc-label {
        font-size: 10px;
        color: #6b7280;
        text-align: right;
    }
    .doc-label strong {
        display: block;
        font-size: 12px;
        color: #111827;
    }

    .info-card {
        background: #f0f7ff;
        border-left: 4px solid #1d4ed8;
        border-radius: 6px;
        padding: 12px 16px;
        margin-bottom: 20px;
    }
    .info-grid {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 6px 24px;
    }
    .info-row { display: flex; gap: 6px; }
    .info-label { color: #6b7280; min-width: 80px; }
    .info-value { font-weight: bold; color: #111827; }

    .rekap-title {
        font-size: 10px;
        font-weight: bold;
        color: #374151;
        margin-bottom: 10px;
        text-transform: uppercase;
        letter-spacing: 0.5px;
    }
    .rekap-grid {
        display: grid;
        grid-template-columns: repeat(5, 1fr);
        gap: 8px;
        margin-bottom: 22px;
    }
    .rekap-box {
        border-radius: 8px;
        padding: 10px 8px;
        text-align: center;
    }
    .rekap-box .num {
        font-size: 20px;
        font-weight: bold;
        line-height: 1;
    }
    .rekap-box .lbl {
        font-size: 9px;
        margin-top: 3px;
        text-transform: uppercase;
        letter-spacing: 0.4px;
    }
    .box-hadir { background: #d1fae5; }
    .box-hadir .num { color: #065f46; }
    .box-hadir .lbl { color: #059669; }
    .box-sakit { background: #fef3c7; }
    .box-sakit .num { color: #78350f; }
    .box-sakit .lbl { color: #d97706; }
    .box-izin  { background: #dbeafe; }
    .box-izin .num  { color: #1e3a8a; }
    .box-izin .lbl  { color: #2563eb; }
    .box-alpha { background: #fee2e2; }
    .box-alpha .num { color: #7f1d1d; }
    .box-alpha .lbl { color: #dc2626; }
    .box-total { background: #f3f4f6; }
    .box-total .num { color: #111827; }
    .box-total .lbl { color: #6b7280; }
    .info-data {
    display: grid;
    grid-template-columns: 1fr 1fr; 
    gap: 10px 40px; 
    margin-bottom: 25px;
    font-size: 11pt;
}

.data-row {
    display: flex;
    justify-content: space-between;
}

.data-label {
    width: 120px;
    color: #555;
}

.data-value {
    font-weight: 600;
    color: #000;
    flex-grow: 1;
}

    .table-title {
        font-size: 10px;
        font-weight: bold;
        color: #374151;
        margin-bottom: 10px;
        text-transform: uppercase;
        letter-spacing: 0.5px;
    }

    table { width: 100%; border-collapse: collapse; }

    thead tr { background: #1d4ed8; color: white; }
    thead th {
        padding: 8px 10px;
        text-align: left;
        font-size: 10px;
        font-weight: bold;
        letter-spacing: 0.3px;
    }
    tbody tr:nth-child(even) { background: #f9fafb; }
    tbody tr:nth-child(odd)  { background: #ffffff; }
    tbody td {
        padding: 7px 10px;
        font-size: 10px;
        color: #374151;
        border-bottom: 1px solid #f3f4f6;
        vertical-align: middle;
    }

    .badge {
        display: inline-block;
        padding: 2px 8px;
        border-radius: 20px;
        font-size: 9px;
        font-weight: bold;
    }
    .badge-hadir { background: #d1fae5; color: #065f46; }
    .badge-sakit { background: #fef3c7; color: #78350f; }
    .badge-izin  { background: #dbeafe; color: #1e3a8a; }
    .badge-alpha { background: #fee2e2; color: #7f1d1d; }
    .badge-bolos { background: #fce7f3; color: #831843; }

    .sessions-badge {
        display: inline-block;
        background: #e0e7ff;
        color: #3730a3;
        font-size: 8px;
        font-weight: bold;
        padding: 1px 5px;
        border-radius: 10px;
        margin-left: 4px;
        vertical-align: middle;
    }

    .date-separator {
        background: #f8fafc !important;
    }
    .date-separator td {
        font-weight: bold;
        font-size: 10px;
        color: #6b7280;
        padding: 6px 10px;
        border-bottom: 1px solid #e5e7eb;
        border-top: 1px solid #e5e7eb;
    }

    .footer {
        margin-top: 30px;
        border-top: 1px solid #e5e7eb;
        padding-top: 14px;
        display: flex;
        justify-content: space-between;
        align-items: flex-end;
    }
    .footer-note {
        font-size: 9px;
        color: #9ca3af;
        line-height: 1.6;
    }
    .ttd {
        text-align: center;
        font-size: 10px;
        color: #374151;
    }
    .ttd .garis {
        margin-top: 48px;
        border-top: 1px solid #374151;
        width: 160px;
        margin-left: auto;
        margin-right: auto;
        padding-top: 4px;
        font-size: 10px;
    }
</style>
</head>
<body>
<div class="page">

    {{-- Header --}}
    <div class="header">
        <div class="header-top">
            <div>
                <div class="school-name">SMK NEGERI 3 JEMBER</div>
                <div class="school-sub">Laporan Kehadiran Siswa</div>
            </div>
            <div class="doc-label">
                Periode
                <strong>{{ $from->format('d M Y') }} &ndash; {{ $to->format('d M Y') }}</strong>
            </div>
        </div>
    </div>

   <table class="info-table">
    <tr>
        <td class="label">Nama Siswa</td>
        <td class="colon">:</td>
        <td class="value">{{ $student->name }}</td>
        
        <td class="label">NISN</td>
        <td class="colon">:</td>
        <td class="value">{{ $student->nisn }}</td>
    </tr>
    <tr>
        <td class="label">Kelas</td>
        <td class="colon">:</td>
        <td class="value">{{ $rombel->name }}</td>
        
        <td class="label">Tahun Ajaran</td>
        <td class="colon">:</td>
        <td class="value">{{ $rombel->academicYear->name }}</td>
    </tr>
    <tr>
        <td class="label">Wali Kelas</td>
        <td class="colon">:</td>
        <td class="value" colspan="4">{{ Auth::user()->name }}</td>
    </tr>
</table>

    {{-- Rekap --}}
    <div class="rekap-title" style="margin-bottom: 10px; font-weight: bold; font-size: 11pt;">Rekapitulasi Kehadiran</div>
        <table class="summary-table">
            <thead>
                <tr>
                    <th>Hadir</th>
                    <th>Sakit</th>
                    <th>Izin</th>
                    <th>Alpha</th>
                    <th>Total Sesi</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>{{ $rekap['hadir'] }}</td>
                    <td>{{ $rekap['sakit'] }}</td>
                    <td>{{ $rekap['izin'] }}</td>
                    <td>{{ $rekap['alpha'] }}</td>
                    <td>{{ $rekap['total'] }}</td>
                </tr>
            </tbody>
        </table>

    <div class="table-title">Detail Kehadiran Per Hari</div>
    <table>
        <thead>
            <tr>
                <th style="width:24px">#</th>
                <th style="width:85px">Tanggal</th>
                <th>Mata Pelajaran</th>
                <th>Guru Pengajar</th>
                <th style="width:80px">Jam</th>
                <th style="width:55px; text-align:center">Status</th>
            </tr>
        </thead>
        <tbody>
            @php $no = 1; @endphp
            @forelse($grouped as $date => $rows)
                <tr class="date-separator">
                    <td colspan="6">
                        {{ \Carbon\Carbon::parse($date)->translatedFormat('l, d F Y') }}
                    </td>
                </tr>
                @foreach($rows as $row)
                    <tr>
                        <td>{{ $no++ }}</td>
                        <td style="color:#9ca3af">&mdash;</td>
                        <td>
                            {{ $row['subject'] }}
                            @if($row['sessions'] > 1)
                                <span class="sessions-badge">{{ $row['sessions'] }} sesi</span>
                            @endif
                        </td>
                        <td style="color:#6b7280">{{ $row['teacher'] }}</td>
                        <td>{{ substr($row['start_time'], 0, 5) }} &ndash; {{ substr($row['end_time'], 0, 5) }}</td>
                        <td style="text-align:center">
                            <span class="badge badge-{{ $row['status'] }}">
                                {{ ucfirst($row['status']) }}
                            </span>
                        </td>
                    </tr>
                @endforeach
            @empty
                <tr>
                    <td colspan="6" style="text-align:center; color:#9ca3af; padding:20px;">
                        Tidak ada data presensi pada periode ini.
                    </td>
                </tr>
            @endforelse
        </tbody>
    </table>
    <div class="footer">
        <div class="footer-note">
            Dicetak pada {{ now()->translatedFormat('d F Y') }}, pukul {{ now()->format('H:i') }} WIB<br>
            Dokumen ini digenerate secara otomatis oleh sistem.<br>
            Catatan: Sesi yang digabung menampilkan status kehadiran terburuk dalam rentang tersebut.
        </div>
        <div class="ttd">
            Wali Kelas,
            <div class="garis">{{ Auth::user()->name }}</div>
        </div>
    </div>

</div>
</body>
</html>