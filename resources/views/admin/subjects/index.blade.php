...
<div class="pb-10 md:p-6" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <form action="{{ route('admin.subjects.index') }}" method="GET" class="flex gap-2 w-full md:w-auto">
            ...
            <a href="{{ route('admin.subjects.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">Reset</a>
        </form>

        <button @click="showModal = true; editMode = false; formUrl = '{{ route('admin.subjects.store') }}'; name=''" 
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-100/50">
                    <th class="p-4 text-sm font-bold text-gray-700">Nama Mata Pelajaran</th>
            ...
            <tbody class="divide-y divide-gray-200/50">
                ...
                <tr class="hover:bg-indigo-50/50 transition-colors">
                    ...
                    <td class="p-4 flex gap-3 justify-end">
                        <button @click="showModal = true; editMode = true; formUrl = '{{ route('admin.subjects.update', $item->id) }}'; name='{{ $item->name }}'"
                                class="text-indigo-600 hover:text-indigo-900 font-semibold text-sm">Edit</button>
                        
                        <form action="{{ route('admin.subjects.delete', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                            @csrf @method('POST')
    ...
    <div x-show="showModal" class="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
        <div class="bg-white/90 backdrop-blur-xl p-8 rounded-2xl w-[400px] shadow-2xl border border-white/30" @click.away="showModal = false">
            <h3 class="text-xl font-extrabold text-gray-900 mb-6" x-text="editMode ? 'Edit Mapel' : 'Tambah Mapel'"></h3>
